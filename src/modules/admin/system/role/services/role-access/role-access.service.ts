...
export class RoleAccessService {
  ...
  async roleToAccess(
    roleId: number,
    roleAccessReqDto: RoleAccessReqDto
  ): Promise<string> {
    return getManager()
  ...
  async allMenus(): Promise<AllMenusVo[]> {
    const menusList: Pick<AccessEntity,
      "id" | "moduleName" | "actionName" | "parentId">[] = await this.accessRepository.find({
    ...
    return menusList.map(
      (
        item: Pick<AccessEntity,
          "id" | "moduleName" | "actionName" | "parentId">
      ) => {
        return {
          id: item.id,
          key: String(item.id),
